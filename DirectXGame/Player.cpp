...
void Player::Initialize(Model* model, Model* model2, ViewProjection* viewProjection, const Vector3& position) {
	// 引数の内容をメンバ変数に記録
	model_ = model;
	meltPlayer_ = model2;
...
void Player::Draw() {
	...
	if (velocity_.y == 0) {
		...
		if (Input::GetInstance()->PushKey(DIK_DOWN)) {
			meltPlayer_->Draw(worldTransform_, *viewProjection_);
		} else {
...
void Player::MapCollisionDetectionUp(CollisionMapInfo& info) {
	...
	if (info.onCeiling_ && info.onWallcontact) {
		velocity_.y = 0.0f;
	}
}
